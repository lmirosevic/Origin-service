...
// constants
var SubscriptionKeepaliveTimeout = 80;// seconds
var HeartbeatEmissionInterval = 10;// seconds
...
var P = function() {
  ...
  this.persistenceGetter = function(channel, callback) {
    logger.info('Stub implementation, you must override this getter function using `setPeristenceKeyValueGetter`');
  ...
  this.refreshSubscriptionKeepaliveForClient = function(client) {
    ...
    _.debounce(storage.cleaners[client], SubscriptionKeepaliveTimeout * 1000);
  };
   ...
   */
  this.sendHeartbeatToClient = function(client) {
    // create an origin packet
    var originPacket = {
      type: 'heartbeat',
      payload: null
  ...
  this.sendChannelUpdateMessageToSubscribersOnChannelInPacketType = function(channel, message, type) {
    ...
    _.each(storage.channels[channel], function(subscriber) {
      ...
      p.sendChannelUpdateMessageToClientInPacketType(subscriber, channel, message, type);
    });
  };
  ...
  this.processIncomingOriginPacketFromClient = function(client, rawOriginPacket) {
    ...
    switch (originPacket.type) {
      ...
      case 'heartbeat': {
        ...
        p.sendHeartbeatToClient(client);
      } break;

      default: {
        logger.info('Got garbage');
      } break;
    }
  };

  this.cyanLog = function(string) {
    logger.info('>>> ' + clc.cyan(string));
  };
};
var p = new P();
...
var ZeroMQ = function() {
  ...
  this.startService = function() {
    if (!serviceStarted) {
      ...
      logger.info('Started ZeroMQ frontend service on port ' + port);
  ...
  this.sendChannelUpdate = function(channel, value) {
    ...
    p.sendChannelUpdateMessageToSubscribersOnChannelInPacketType(channel, value, 'update');
  };
